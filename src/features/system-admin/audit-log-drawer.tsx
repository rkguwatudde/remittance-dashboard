...
import type { AdminPlatformAuditLogRow } from "@/lib/remittance-admin-api";
...
export function AuditLogDrawer({
  ...
  open: boolean;
  log: AdminPlatformAuditLogRow | null;
  onClose: () => void;
}) {
  const [mounted, setMounted] = React.useState(false);
  const holdRef = React.useRef<AdminPlatformAuditLogRow | null>(null);
          ...
          >
            <div className="flex items-start justify-between gap-3 border-b border-border px-4 py-3">
              <div className="min-w-0">
                <p className="text-sm font-semibold text-foreground">Audit event</p>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
              <dl className="space-y-2 text-sm">
                ...
                <div className="grid gap-1 sm:grid-cols-[120px_1fr]">
                  ...
                  <dd className="font-mono text-xs">{d.action}</dd>
                </div>
                <div className="grid gap-1 sm:grid-cols-[120px_1fr]">
                ...
                <div className="grid gap-1 sm:grid-cols-[120px_1fr]">
                  ...
                  <dd className="font-mono text-xs break-all">
                    {d.actor_type} {d.actor_id ? `· ${d.actor_id}` : ""}
                  </dd>
                </div>
                <div className="grid gap-1 sm:grid-cols-[120px_1fr]">
                  <dt className="text-xs font-medium text-muted-foreground">User id</dt>
                  <dd className="font-mono text-xs break-all">{d.user_id ?? "—"}</dd>
                </div>
                <div className="grid gap-1 sm:grid-cols-[120px_1fr]">
                  <dt className="text-xs font-medium text-muted-foreground">Device</dt>
                  <dd className="font-mono text-xs">{d.device ?? "unknown"}</dd>
                </div>
                <div className="grid gap-1 sm:grid-cols-[120px_1fr]">
              ...
              <div>
                ...
                <DiffViewer oldValues={d.old_values} newValues={d.new_values} />
              </div>
